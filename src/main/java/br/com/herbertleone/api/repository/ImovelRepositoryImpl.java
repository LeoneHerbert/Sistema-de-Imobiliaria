...
public class ImovelRepositoryImpl implements ImovelRepository {
	...
	public Imovel buscaPorId(Long id) {
		return manager.createQuery("SELECT imovel FROM Imovel imovel WHERE imovel.id = :pId", Imovel.class)
				.setParameter("pId", id)
				.getSingleResult();
	}
	...
	public Imovel buscaPorTipo(String tipo) {
		return manager.createQuery("SELECT imovel FROM Imovel imovel WHERE imovel.tipoDeImovel = :pTipo", Imovel.class)
